                ...
                {sortedPlayers.map((player, index) => {
                    ...
                    return (
                        <div key={player.id} className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${isMe ? 'bg-sky-500/20 border-sky-500/50' : 'bg-slate-800/50 border-white/5'}`}>
                            ...
                            <div>
                                {player.won ? (
                                    <span className="bg-green-500/20 text-green-400 font-black px-3 py-1 rounded-lg text-xs uppercase tracking-widest border border-green-500/30">Venceu</span>
                                ) : !player.isAlive ? (
